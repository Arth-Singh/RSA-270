#include "md1.hpp"

#include <algorithm>
#include <stdexcept>

namespace rsa270 {
namespace {

constexpr std::array<std::uint8_t, 256> kSbox = {
    ,
};

}  // namespace

Md1::Md1() = default;

void Md1::update_byte(std::uint8_t value) {
  state_[index_ + 16] = value;
  state_[index_ + 32] = state_[index_] ^ value;
  checksum_[index_] ^= kSbox[value ^ last_];
  last_ = checksum_[index_];
  index_ = (index_ + 1) & 0x0f;

  if (index_ != 0) return;

  std::uint8_t t = 0;
  for (int round = 0; round < 18; ++round) {
    for (int position = 0; position < 48; ++position) {
      const auto countdown = static_cast<std::uint8_t>(48 - position);
      t = state_[position] ^ kSbox[static_cast<std::uint8_t>(t + countdown)];
      state_[position] = t;
    }
  }
}

void Md1::update(std::span<const std::uint8_t> input) {
  if (finalized_) throw std::logic_error("MD1 context already finalized");
  for (const auto value : input) update_byte(value);
}

std::array<std::uint8_t, 16> Md1::final() {
  if (finalized_) throw std::logic_error("MD1 context already finalized");

  const auto padding = static_cast<std::uint8_t>(16 - index_);
  for (int i = 0; i < padding; ++i) update_byte(padding);
  for (int i = 0; i < 16; ++i) update_byte(checksum_[i]);

  finalized_ = true;
  std::array<std::uint8_t, 16> result{};
  std::copy_n(state_.begin(), result.size(), result.begin());
  return result;
}

std::array<std::uint8_t, 16> Md1::digest(
    std::span<const std::uint8_t> input) {
  Md1 md1;
  md1.update(input);
  return md1.final();
}

}  // namespace rsa270
