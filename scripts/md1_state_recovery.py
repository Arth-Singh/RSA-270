        ...
        while process.poll() is None:
            ...
            if marker.exists():
                try:
                    ...
                    continue
                if not announced:
                    # This environment may expose process-relative monotonic
                    # origins. Use the parent's own observation as its origin;
                    # never subtract the child's recorded monotonic value.
                    observed_solve_start = time.monotonic()
                    print("SOLVING fixture-0 full_unknown_bits=128 full_sweeps=18 "
                          "native_timeout_ms=59000 external_cap_seconds=60", flush=True)
                    announced = True
                if time.monotonic() - observed_solve_start >= 59.9:
                    process.kill()
                    process.wait()
                    if not (output / "solver-result.json").exists():
                        write_json(output / "solver-result.json", {
                            ...
                            "solve_seconds": time.monotonic() - observed_solve_start,
                            "statistics": None,
                        })
                    break
